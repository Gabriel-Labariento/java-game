...
public class Spider extends Enemy{
    ...
    public void updateEntity(ServerMaster gsm){
        ...
        if ((getDistanceBetween(this, pursued) <= GameCanvas.TILESIZE * 4)) {
        ...
        if (now - lastSpriteUpdate > SPRITE_FRAME_DURATION) {
            if (worldX > pursued.getWorldY()) {
                currSprite++;
                if (currSprite > 1) currSprite = 0;
            } else {
                currSprite++;
                if (currSprite < 2 || currSprite > 3) currSprite = 2;
            }
